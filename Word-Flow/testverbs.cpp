    ...
    ui(new Ui::TestVerbs)
{
    ...
    QPixmap myPixmap(":/new/prefix1/res/glag.jpg");
...
void TestVerbs::FillStart(){
    srand(time(nullptr));
    for(int i = 0; i < 10; i++){
        numbers_words[i].question_number = rand() % 2 + 1;
    }
    for(int i = 0; i < 10; i ++){
        numbers_words[i].word_answer_number = 200 + rand() % 100 + 1;
        for(int j = 0; j < i; j ++){
    ...
    for(int i = 0; i < 10; i++){
        numbers_words[i].word_1_number = 200 + rand() % 100 + 1;
        for(int j = 0; j < 10; j ++){
    ...
    for(int i = 0; i < 10; i++){
        numbers_words[i].word_2_number = 200 + rand() % 100 + 1;
        for(int j = 0; j < 10; j ++){
    ...
    for(int i = 0; i < 10; i++){
        numbers_words[i].word_3_number = 200 + rand() % 100 + 1;
        for(int j = 0; j < 10; j ++){
...
void TestVerbs::on_start_Button_clicked()
{
    srand(time(nullptr));
    QMessageBox msgBox;
...
void TestVerbs::on_back_to_menu_Button_clicked()
{
    this->close();
    emit VerbsMenu();
}

void TestVerbs::on_next_Button_clicked()
{
    QButtonGroup group;
    ...
    if ((group.checkedId() == -1)||(group.checkedId() == 4)){
        ...
        db.setDatabaseName("/home/timowka0304/Word-Flow/Word-Flow/Word-Flow/Word-Flow/Words.db3");
        db.open();
...
void TestVerbs::on_done_Button_clicked()
{
    QSqlDatabase db;
    db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName("/home/timowka0304/Word-Flow/Word-Flow/Word-Flow/Word-Flow/Progress.db3");
    db.open();

    QSqlQuery query;
    query.exec(QStringLiteral("INSERT INTO Progress VALUES (%1, %2, 10, 'Глаголы')").arg(sum*10).arg(sum));
...
void TestVerbs::Warning(int flag, QString eng, QString rus){
    QMessageBox msgBox;
